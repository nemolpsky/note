...
- 因为是实现了```Deque```接口的缘故，有很多队列的方法，很多方法只是不同名字而已，只要记住一个原则，就是所有对链表头部和尾部的操作时间复杂度一定是O(1)，因为内部有两个变量记录了头尾两个节点，但是只要涉及到是对链表中部做操作，查询、插入和删除都必定是O(n)，因为这些操作都需要先进行查找，而链表的查找只能是遍历。
