...
Java语言和C语言最大的区别就是由JVM来控制内存的建立和释放，也称作垃圾回收(GC)，JVM会把堆内存分为新生代和老年代，一般新创建的对象会进入新生代，新生代的垃圾回收被称为Minor GC，经过一次Minor GC后还存活着的对象年龄会加1，达到阀值(默认15)之后对象会倍放入老年代，老年代也会触发垃圾回收，被称为Full GC，这种回收会更加耗时，这样分代也是为了提高回收的效率。

要注意的是，GC不仅仅是清理对象释放内存，还需要整理内存。像下图所示，随着使用和清理，整个内存的可用区域变得不连续，如果想要在内存上连续存储一个数组那就会找不到一块连续可用的区域，因此GC除了清理对象之外还需要整理内存，把这种间隔的可用内存区域整理成一块连续完整的，这种操作才是最耗时的，而且对象的地址也会跟着变化，所以整理期间这些对象对外是不可用的，这也造成了常说的GC停顿。
