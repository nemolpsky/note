...
3. join更新、删除数据
   ...
   mysql5.6中引入了Materialization(物化)这一关键特性用于子查询优化(比如在IN/NOT IN子查询以及FROM子查询，大致可以理解为将子查询的结果存储为一个临时表来使用，避免每条数据执行where判断的时候都是去重复执行子查询)，但是只对查询起作用，更新和删除必须改成join的格式，否则会很慢。
...
4. 条件下推
   ...
   很多时候会写出这种在子查询中分组筛选后，再根据子查询的结果继续筛选的语句，其实可以直接在子查询中完成条件筛选，速度会快很多，可以看到一个是直接利用索引查询，另一个是子查询全表后还继续在子查询中查询。
...
5. 相关名词介绍
   ...
   - 条件下推
     ...
     尽量将筛选条件放到子查询中，可以大大加快查询效率，其原理是在物化的基础上利用子查询尽量过滤掉多余的数据，然后外层的查询又都会利用到物化子查询中的临时表，所以效率会大大提高。
